...
vec3 processLight(vec3 normalInterp, vec3 vertPos, vec3 difColor, vec3 specCol){
  ...
  vec3 colorLinear = ambientColor +
                     difColor * lambertian * lightColor * lightPower / distance +
                     specCol * specular * lightColor * lightPower / distance;
...
void fragment(){
	...
	if(d<MAX_DIST){
		...
		vec3 n = GetNormal(p);
		vec3 difCol;
		difCol.x = orbitTrap(vec3(0,0,1), p);
		difCol.y = orbitTrap(vec3(0,1,0), p);
		difCol = difCol/6.0;
		vec3 specCol = difCol;
		col = processLight(n,p,difCol,specCol);
	}
	else{
		ALPHA = 0.0
	}
